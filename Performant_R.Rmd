...
xaringanExtra::use_xaringan_extra(c("panelset", "scribble", "tile_view", "editable","share_again"))
```
...
plot(qrcode::qr_code("https://richardjacton.github.io/"))
```
...
- How?
	...
	- Other Simple marking tools e.g. {[bench](https://github.com/r-lib/bench)} & {[tictoc](https://collectivemedia.github.io/tictoc/)}
...
Profiling the larger codebase for simulating the same revealed that rolling the dice was taking up a lot of the total run time.
...
- In R (almost) everything is a vector
	- Rule of Thumb: vector is faster than loop
...
- Applies to matrices/arrays too as these are secretly vectors
		- Matrix arithmetic/linear algebra is very fast if you can cast your problem in these terms you can get a speedup
...
cppFunction("int move_square(int current) {
	...
	if(isDouble) {
		...
		} else { current += sum(rolls) + sum(rolls2); return(current); }
	}
	current += sum(rolls);
...
devtime vs runtime
https://www.youtube.com/watch?v=ttJCwzfsAc8&list=TLPQMDMwNTIwMjK5d2liOjkIEQ&index=2
...
- https://www.youtube.com/watch?v=78icyDMZJyQ
...
note: `r format(object.size(initial), "Kb")`
